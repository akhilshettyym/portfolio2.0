          ...
          <div className="flex h-full w-full items-center justify-center px-6">
            <div className="max-w-6xl text-center">
              <motion.div initial={{ opacity: 0, y: 18 }}
                ...
                <span className="inline-block overflow-hidden"> {word}
            ...
            <div className="relative h-65 w-full max-w-6xl text-center">
              <motion.div initial={{ opacity: 0, scale: 1.05, y: 20, filter: "blur(12px)" }}
                ...
                <div className="text-[clamp(2.8rem,8vw,3rem)] font-semibold tracking-tight whitespace-nowrap">
                  <CurtainText> {" "} My name is <span className="font-bold"> AKHIL </span> </CurtainText>
                </div>
              </motion.div>

              <AnimatePresence>
                {nameStage >= 1 && (
                  <motion.div initial={{ opacity: 0, y: 30, filter: "blur(10px)" }}
        ...
        <SceneShell dark={false}>
          ...
          <motion.div initial={{ opacity: 0, scale: 1.03, filter: "blur(16px)" }}
            ...
            {HISTORYBANDS.map((band, index) => {
              ...
              const rowText = Array.from({ length: 12 })
                ...
                <motion.div key={band.year}
                  ...
                  <motion.div animate={{ x: `${x}vw` }} transition={{ duration: 0.08, ease: "linear" }} className="flex h-full items-center whitespace-nowrap px-4 md:px-10">
                    ...
                    <div ref={(el) => {
                      ...
                      <motion.span initial={{ opacity: 0, x: -40 }}
                        animate={{ vopacity: timelineReveal ? 1 : 0, vx: timelineReveal ? 0 : -40 }}
            ...
            <motion.div className="pointer-events-none absolute inset-x-6 bottom-6 h-1 rounded-full bg-black/10">
              <motion.div className="h-full origin-left bg-black"
            ...
            <AnimatePresence mode="wait">
              {butStage < 2 ? (
                <motion.div key="but"
                  ...
                  <div className="text-[clamp(3rem,8vw,4rem)] font-bold tracking-tight text-white">
                    {" "} BUT...{" "}
                  </div>
                </motion.div>
              ) : (
      ...
      return (
        <SceneShell dark={finalStage === 0}>
          <motion.div initial={false}
            animate={{ opacity: finalStage === 0 ? 1 : 0, scale: finalStage === 0 ? 1 : 1.02 }}
