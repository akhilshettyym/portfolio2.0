...
function GlitchNavItem({ href, label, active, delay = 0 }) {
  ...
  return (
    <Link href={href} onMouseEnter={handleEnter} onMouseLeave={handleLeave} className={`px-2 py-2 text-[11px] font-bold uppercase tracking-tight transition text-center inline-flex justify-center items-center min-w-10 ${active ? "bg-black text-white" : "bg-gray-200/90 text-gray-600 hover:bg-black hover:text-white"}`} style={width ? { width: Math.max(width, 60) } : {}}>
