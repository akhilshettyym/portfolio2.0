  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl w-full mx-auto bg-white">
      <div className="flex justify-between items-center border-b border-black pb-4 mb-6">
        <h1 className="font-mono font-semibold uppercase tracking-normal text-lg">
          Inbound Leads
        </h1>
      ...
      {leads.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-6">
          {leads.map((lead, index) => (
            <div
              key={lead._id || `${lead.email || "lead"}-${lead.createdAt || index}`}
              className="border border-black p-4 flex flex-col md:flex-row justify-between items-start md:items-stretch gap-4 bg-white"
            >
              <div className="space-y-2 flex-1 w-full">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span
                    className={`text-[10px] font-mono uppercase tracking-widest px-3 py-1 ml-auto md:ml-0 border border-black ${lead.purpose === "work" ? "bg-black text-white" : "bg-white text-black"}`}
                  >
                ...
                {lead.purpose === "work" && (
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 py-2 border-t border-b border-dashed border-black/40 text-xs font-mono">
                    <div>
                      ...
                      <span className="uppercase">{lead.projectType}</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Est. Budget:</span>{" "}
                      <span className="uppercase">{lead.budget?.replace("_", " ")}</span>
                    </div>
                    {lead.deadline && (
                      <div>
              ...
              <div className="flex items-end md:items-center justify-end w-full md:w-auto pt-2 md:pt-0 border-t md:border-t-0 md:border-l border-dashed border-black/40 md:pl-4">
                <button
                  onClick={() => initiateDelete(lead._id)}
                  className="w-full md:w-auto border border-black text-black px-4 py-2 md:py-1 text-xs font-mono uppercase hover:bg-black hover:text-white transition-colors duration-150"
                >
                  Delete
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-sm bg-white p-6 border border-black">
            ...
            <p className="mt-2 text-sm text-gray-600 font-sans">
              Are you sure you want to delete this lead permanently? This action cannot be
              undone.
            </p>
