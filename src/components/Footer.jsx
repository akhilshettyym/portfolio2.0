...
const Footer = () => {
    ...
    useEffect(() => {
        ...
        const observer = new IntersectionObserver(
            ...
            { threshold: [0, 0.12, 0.22, 0.38, 0.55, 0.75, 1] }
        );
    ...
    return (
        <motion.section ref={sectionRef}
            ...
            <div className="pointer-events-none absolute inset-0 overflow-hidden">
                <motion.div style={{ opacity: glowOpacity }} className="absolute left-1/2 top-10 h-[32rem] w-[32rem] -translate-x-1/2 rounded-full bg-white blur-3xl sm:h-[46rem] sm:w-[46rem]" />
            ...
            <motion.div style={prefersReducedMotion ? undefined : { y: revealLift, boxShadow: shellShadow }}
                animate={prefersReducedMotion ? undefined : { boxShadow: shouldSnap ? "0 40px 100px rgba(0,0,0,0.22)" : "0 14px 44px rgba(0,0,0,0.08)" }} transition={{ duration: 0.75, ease: [0.22, 1, 0.36, 1] }} className="relative mx-auto min-h-[100svh] w-full overflow-hidden rounded-none bg-white">
                <motion.div aria-hidden="true" className="pointer-events-none absolute inset-0 z-10 bg-[linear-gradient(to_bottom,rgba(255,255,255,0.08),rgba(255,255,255,0.02)_20%,transparent_45%)]" style={{ opacity: useTransform(scrollYProgress, [0, 0.2, 0.5, 1], [0.7, 0.5, 0.2, 0]) }} />

                <motion.div aria-hidden="true" className="pointer-events-none absolute inset-x-0 top-0 z-20 h-px bg-black/10" style={{ opacity: useTransform(scrollYProgress, [0, 0.18, 0.45, 1], [0.85, 0.5, 0.18, 0]) }} />
                                                ...
                                                <div className="flex flex-row gap-4 h-full w-full">
                                                    ...
                                                    <div className="flex-1 bg-gray-200 rounded-md p-4">
                                                        <div className="absolute top-4 right-5 z-10 bg-gray-200">
                                                            <Image src="/footer/animated_zigzag.gif" alt="Animated zigzag pattern" width={350} height={20} priority unoptimized className="object-contain mix-blend-multiply" />
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            <div className="flex h-full w-full flex-col gap-4 bg-white p-6 shadow-sm md:w-[40%]">
                                <div className="h-[35%] bg-slate-50 rounded-xl p-4 flex flex-row gap-4 w-full">
                                    <div className="w-[50%] p-3">
                                        <Image src="/footer/animated_blob_gloop.gif" alt="animated blob gloop" width={380} height={35} priority unoptimized className="object-contain mix-blend-multiply" />
                                    </div>
