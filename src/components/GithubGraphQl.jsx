        ...
        <motion.div initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.45 }} className="w-full p-8">
            ...
            <div className="mb-5 grid grid-cols-3 gap-4 uppercase">
                <div className="relative flex flex-col items-center justify-center px-5 py-5 text-center">
                    <AnimatePresence>
                        {aiNotif && (
                            <motion.div key={aiNotif.id} initial={{ opacity: 0, y: 8, scale: 0.8 }} animate={{ opacity: 1, y: -12, scale: 1 }} exit={{ opacity: 0, y: -24, scale: 1.9 }} transition={{ duration: 1.5, ease: "easeOut" }} className="absolute top-3 right-33 text-[11px] font-semibold text-zinc-500">
                                +{aiNotif.value}
                            </motion.div>
                        )}
                    </AnimatePresence>
                ...
                <div className="relative flex flex-col items-center justify-center px-5 py-5 text-center">
                    <AnimatePresence>
                        {coffeeNotif && (
                            <motion.div key={coffeeNotif.id} initial={{ opacity: 0, y: 8, scale: 0.8 }} animate={{ opacity: 1, y: -12, scale: 1 }} exit={{ opacity: 0, y: -24, scale: 0.9 }} transition={{ duration: 1.2, ease: "easeOut" }} className="absolute top-3 right-35 text-[11px] font-semibold text-zinc-500"> +{coffeeNotif.value} </motion.div>
                        )}
                    </AnimatePresence>
                ...
                <div className="relative flex flex-col items-center justify-center px-5 py-5 text-center">
                    <AnimatePresence>
                        {commitNotif && (
                            <motion.div key={commitNotif.id} initial={{ opacity: 0, y: 8, scale: 0.8 }} animate={{ opacity: 1, y: -12, scale: 1 }} exit={{ opacity: 0, y: -24, scale: 0.9 }} transition={{ duration: 1.2, ease: "easeOut" }} className="absolute top-3 right-36 text-[11px] font-semibold text-zinc-500"> +{commitNotif.value} </motion.div>
                        )}
                    </AnimatePresence>
            ...
            <div className="mb-5 text-center">
                <h2 className="text-[28px] font-semibold tracking-tight text-black uppercase"> My - Personal GitHub Activity </h2>
                <p className="mt-1 text-sm italic text-zinc-500"> (Work commits are hiding in another dimension) </p>
            </div>
                ...
                {loading ? (
                    <div className="flex h-65 items-center justify-center">
                        <div className="flex items-center gap-3">
                            ...
                            <p className="text-sm font-medium text-zinc-500"> Loading contributions... </p>
                        </div>
                    </div>
                ) : (
                    <>
