...
export default function Loader({ onFinish }) {
  ...
  /**
   * Pick a random point at which to ask for location preference.
   */
  useEffect(() => {
    pausePointRef.current = Math.floor(Math.random() * 30) + 20;
  }, []);
  ...
  /**
   * Loader progress / location preference flow.
   *
   * `denied` is expected to be treated as an existing preference
   * by hasLocationPreference().
   */
  useEffect(() => {
    let frameId;
  ...
  /**
   * Animate loader out after completion.
   */
  useEffect(() => {
    if (!done) return;
  ...
  /**
   * Called by LocationModal after Accurate, Fast, or Deny.
   */
  const handleLocationSelected = () => {
    setShowLocationModal(false);
