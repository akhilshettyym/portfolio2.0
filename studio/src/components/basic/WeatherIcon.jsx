...
//         <motion.div
//           animate={{ y: moonPhase === "FULL_MOON" ? [0, -1, 0] : 0 }}
...
export default function WeatherIcon({ onClick }) {
  ...
  const WeatherGlyph =
    WEATHER_MAP[sceneCondition]?.icon || TiWeatherSunny;

  const MoonGlyph =
    MOON_MAP[moonPhase]?.icon || WiMoonAltFull;

  const weatherLabel =
    WEATHER_MAP[sceneCondition]?.label || "Weather";

  const moonLabel =
    MOON_MAP[moonPhase]?.label || "Moon";
