...
function MobileProjectModal({ project, onClose, isCompactDevice, isLargeDevice, theme }) {
 ...
 const isNewProject =
   !project.description || project.description.trim().toLowerCase() === "new project";
 const hasDetails = !isNewProject;
 const hasImage = Boolean(project.image) && !skipImage;

 const { modalBg, overlayBg, borderClass, textMuted, textBody, tagClass, closeBtn, ctaBtn } =
   getWorkMobile(theme);
     ...
     <motion.div
       ...
       <div className={`flex items-center justify-between border-b p-4 ${borderClass}`}>
         <div className="flex flex-col">
           <span className={`text-[10px] uppercase tracking-[0.25em] ${textMuted}`}>
             Selected Work
           </span>
       ...
       <div className={`flex-1 p-5 ${skipImage ? "" : "overflow-y-auto"}`}>
         {hasDetails ? (
           <div className="flex flex-col gap-5">
             {hasImage && (
               <div
                 className={`relative aspect-video w-full overflow-hidden rounded-xl border ${borderClass}`}>
                 <Image
                   src={project.image}
                   alt={project.title}
                   fill
             ...
             <>
               ...
               <div className="absolute inset-0 flex flex-col justify-between p-7 z-10">
                 ...
                 <div>
                   ...
                   <p className={`mt-4 max-w-145 text-sm leading-relaxed ${textBody}`}>
                     {project.description}
                   </p>
                   {project.stack && project.stack.length > 0 && (
