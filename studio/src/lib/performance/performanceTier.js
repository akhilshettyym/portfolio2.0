...
export function probeGPUInfo() {
  ...
  try {
    ...
    console.warn(
      "PerformanceEngine: Context initialization rejected by high-performance hardware restriction rules:",
  ...
  try {
    ...
    console.warn(
      "PerformanceEngine: Debug strings masked out by browser canvas fingerprinting protection settings:",
...
export function classifyPerformanceTier({ gpu, fps, p95FrameMs, cpuOps }) {
  ...
  if (
    ...
    /RTX\s?30|RX\s?(6[67]\d{2}|5700)|GTX\s?1080|Vega|Iris\s?Xe|Apple\s?M1|Apple\s?M2|Adreno\s?8[78]|Mali.*G7[78]/i.test(
      gpuText,
    )
  ) {
    score += 24;
